...
3. 观察画布自动创建 CSV 输入节点。
4. 无需单击运行，直接在底部**数据**页查看 10 行源数据、字段表头和首条订单。
5. 观察状态显示文件名和字节数，项目变为“未保存”。
...
4. 依次单击 CSV 输入、筛选和 CSV 输出节点，确认底部表格切换为对应处理阶段的数据。
5. 切换到**字段**，确认 `amount` 推断为 number 或因混合值推断为 string，并解释类型推断基于实际数据。
6. 切换到**日志**，展示 `run-started`、`node-result`、`run-completed` 等事件。
7. 切换到**错误**，正常流程应显示 0。
...
- 当前 118 项单元/组件测试，以及 Chromium、Firefox、WebKit 和移动 Chromium 共 24 项 E2E 已通过。
- 当前 UI 文件选择器只支持 CSV，配置支持可视化与 JSON 双模式，导出按钮当前固定输出 CSV。
...
1. 导入订单 CSV。
2. 添加筛选节点。
3. 保持右侧**可视化**模式；新节点默认已经是“两项运算（加减乘除/比较）”，把运算符改为“不等于（≠）”。
...
在筛选后添加**计算字段**。新节点会直接显示“字段 + 0”：

1. 运算符可直接选择加、减、乘、除。
2. 左值选择字段并填写 `amount-5`。
3. 右值选择“两项运算（加减乘除/比较）”，运算符选“加（+）”。
4. 右值的左侧固定值填 `1`，右侧选择字段并填写 `tax-rate-6`。
5. 新字段名改为“含税金额”，应用配置。
6. 切换到 JSON 后，等价配置如下：
