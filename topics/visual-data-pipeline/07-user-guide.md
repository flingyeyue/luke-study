...
- **选择 CSV**：把文件绑定到一个尚未绑定的 CSV 输入节点；没有可用输入节点时自动创建一个。
...
配置使用 JSON 编辑。修改后必须单击**应用**；只有通过对应 Zod Schema 校验的配置才会写入节点。错误列表会显示配置路径和原因。

### 底部运行面板

- **数据**：显示当前末端节点的最多 1,000 行预览和总行数；表格采用窗口化渲染。
- **字段**：显示字段名、推断类型以及是否可空。
...
筛选条件是受限表达式 AST，不执行 JavaScript 字符串。
...
1. 单击**选择 CSV**并选择文件，系统创建 CSV 输入。
2. 依次添加类型转换、筛选、计算字段、排序或其他节点。
3. 逐个选中节点，编辑配置 JSON 并单击**应用**。
4. 添加 CSV 输出节点。
5. 单击**运行**，查看数据、字段、日志和错误。
6. 单击**导出**下载当前预览结果。
...
- 配置面板是 JSON 编辑器，不是每种节点的专用表单。
- 顶部导出命令当前固定输出 CSV，没有按画布输出节点类型切换格式。
- 列 ID 暂不在字段页签直接显示，需要按字段名和序号推导。
