...
完整功能和配置字段参考见[使用与功能指南](07-user-guide.md)。本手册强调操作顺序和演示话术。
...
4. 单击**应用**，确认“配置已应用”。
5. 添加 CSV 输出并运行。
6. 预期剩余 8 行，`ORD-1002` 和 `ORD-1008` 不再出现。
...
> 当前版本完成了可演示 MVP，同时明确保留 JSON/XLSX UI、专用配置表单、完整大结果导出和同步解析取消等限制，没有把底层库能力包装成已经交付的用户功能。
